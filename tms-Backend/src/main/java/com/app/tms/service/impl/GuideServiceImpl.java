...
public class GuideServiceImpl implements GuideService {
	...
	public GuideDto getGuideById(Long guideId) {
		Guide guide = guideRepository.findById(guideId)
				.orElseThrow(() -> new ResourceNotFoundException("Guide does not exist with given Id : " + guideId));
	...
	public List<GuideDto> getAllGuides() {
		...
		return guides.stream().map((guide) -> GuideMapper.mapToGuideDto(guide)).collect(Collectors.toList());
	}

	@Override
	public GuideDto updateGuide(Long guideId, GuideDto updatedGuide) {
		Guide guide = guideRepository.findById(guideId)
				.orElseThrow(() -> new ResourceNotFoundException("Guide does not exist with id : " + guideId));
